...
{
	bool clip = ymin < ymax;
	if (channel < 1 || channel > my ny) return Melder_errorp1 (L"Channel does not exist.");
	if (tmin >= tmax)
	{
		tmin = my xmin;
		tmax = my xmax;
	...
	if (tmin >= my xmax || tmax < my xmin) return Melder_errorp1 (L"Invalid domain.");
	...
	Polygon him = Polygon_create (numberOfPoints);
...
Polygon Sounds_to_Polygon_enclosed (Sound me, Sound thee, int channel, double tmin, double tmax, double ymin, double ymax)
{
	...
	if (my ny > 1 && thy ny > 1 && my ny != thy ny)
		return Melder_errorp ("The numbers of channels of the two sounds have to be equal or 1.");

	long numberOfChannels = my ny > thy ny ? my ny : thy ny;

	if (channel < 1 || channel > numberOfChannels) return Melder_errorp1 (L"Channel does not exist.");
	...
	if (xmax <= xmin) return Melder_errorp1 (L"Domains must overlap.");
	if (tmin >= tmax)
	{
		tmin = xmin;
		tmax = xmax;
	...
	if (tmin >= xmax || tmax < xmin) return Melder_errorp1 (L"Invalid domain.");
